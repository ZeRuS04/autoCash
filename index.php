...
<body>
	...
	<div id="Menu">
		...
		<form name = "add_repair" action = "/add_repair.php" method = "get">
			<input type = "submit" name = "add_repair" value = "Добавить ремонт">
		</form>
	...
	<div id="SrchForm" >
		...
		<table name="tbl">
			<tr>
				...
				<td>
					<input type = "text" name = "order_num" value = "№ заказ-наряда"  onclick="value=''">;
				</td>
				<td>
					<input type = "text" name = "date" value = "Дата(гггг-мм-дд)" onclick="value=''">;
				</td>
				<td rowspan="3">
				<div align="center" id="Button"> 
						
						<form name = "search_clr" action = "/index.php" method = "get">				
							<input type = "submit" name = "clearbut" value = "Сбросить">			
						</form>
			...
			<tr>
				...
				<td>
					<input type = "text" name = "name" value = "ФИО" onclick="value=''">;
				</td>
				<td>
					<input type = "text" name = "license_num" value = "№ Вод.удостоверения" onclick="value=''">;
				</td>
			</tr>
			<tr>
				<td> 
				...
				<td>
					<input type = "text" name = "gang_num" value = "№ Бригады" onclick="value=''">;
				</td>
				<td>
					<input type = "text" name = "specialization" value = "Специализация бригады" onclick="value=''">;
				</td>
			</tr>
				<td> 
					Автомобиль:	
				...
				<td>
					<input type = "text" name = "model" value = "Марка(модель)" onclick="value=''">;
				</td>
				<td>
					<input type = "text" name = "state_number" value = "Гос.номер" onclick="value=''">;
				</td>
				<td>
					<input type = "text" name = "VIN" value = "VIN" onclick="value=''">;
				</td>
			<tr>
		...
		if($total > 0)
		{
			...
			for( $i = 0;  $i <$end; $i++)
			{
				...
				echo "
					...
					<td>".$line[$i]['VIN']."</td>
					<td>
						<form name = 'del' action = '/edit.php' method = 'get'>
							<input type='hidden' name='order_num' value='$delnum'>
							<input type='hidden' name='edit' value=''>  
							<input type='image' src='/images/edit.png' alt='edit'>
						</form>	
						</td>
						<td>
						<form name = 'del' action = '/print.php' method = 'get'>
							<input type='hidden' name='order_num' value='$delnum'>
							<input type='image' src='/images/print.png' alt='pt'>
						</form>	
						</td>
						<td>
						<form name = 'del' action = '/index.php' method = 'get'>
							...
							<input type='image' src='/images/delete.png' alt='del'>
						</form>
						</td>
					<tr>
				";
...
$(function(){                       // после загрузки страницы
   $('button').click(function(){// на нажатие h1 вешаем обработчик, который
     $('#SrchForm').toggle('slow');        // у элемента с #toggler переключет видимость
   });
});
</script>
</body>
